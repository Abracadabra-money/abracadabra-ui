    ...
    <div class="stake-wrap">
      <div class="actions-block">
        ...
        <div class="action-form">
          <h4 class="sub-title">Select amount</h4>
          ...
          <div class="unlock-info">
            <div class="info-row">
              <div class="info-title">Spell to be recieved</div>
              <div class="info-value">
                ...
                />
                {{ formatTokenBalance(Number(inputValue)) }}
              </div>
            </div>
            <div class="info-row">
              <div class="info-title">Unlocks on</div>
              <div class="info-value">{{ unlocks }}</div>
            </div>
          </div>
        ...
        <div class="claim-info">
          <h4 class="sub-title">MIM to Claim</h4>
      ...
      <div class="stake-info">
        ...
        <div class="lock-info">
          <div>
            ...
            <p class="lock-text">
              Locked funds will be automatically converted to Staking status
...
export default {
  data() {
    return {
      ...
      stakeData: null as any | SpellLockInfo,
      updateInterval: null as null | NodeJS.Timeout,
      specialInfo: [
        {
          text: `Stake your SPELL into mSPELL! No impermanent loss, no loss of governance rights. Take part in the fee sharing mechanism of Abracadabra and earn MIM! Find out more `,
          link: "https://docs.abracadabra.money/learn/intro/stake/mspell",
          subText: `sSPELL automatically earns fees from MIM repayments from all wizards proportional to your share of the stake pool.`,
        },
      ],
    };
  },
  ...
  computed: {
    ...
    userLocks() {
      ...
      return this.stakeData.lockInfo.userLocks.map(
        ({ unlockTime, amount }: UserLock) => {
    ...
    claimAmount() {
      return formatUnits(this.stakeData?.lockInfo?.claimAmount || 0n, 18);
    },

    lockAmount() {
      return formatUnits(this.stakeData?.lockInfo?.lockAmount || 0n, 18);
    },

    isClaimDisabled() {
      return !Number(this.claimAmount);
    },
    ...
    unlocks() {
      return moment
        .unix(
          Number(moment().unix()) +
            Number(this.stakeData?.lockInfo?.lockDuration || 0)
        )
        .format("DD MMM YYYY");
    },
  },
  ...
  methods: {
    ...
    async claimHandler() {
      ...
      // @ts-ignore
      const { error } = await claim(this.stakeData.tokenBank);
    ...
    async createStakeInfo() {
      this.stakeData = await getLockInfo(this.account, this.selectedNetwork);
    },
  },
