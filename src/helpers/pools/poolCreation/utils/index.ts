...
export const invertIValueBasedOnUpdatedDecimals = (
  currentI: bigint,
  newDecimals: number,
  oldDecimals: number
) => {
  return (
    (currentI * parseUnits("1", newDecimals)) / parseUnits("1", oldDecimals)
  );
};

export const calculateQuoteAndBaseAmounts = (
  amount: bigint,
  type: TokenTypes,
  IforCalc: bigint,
  baseDecimals: number,
  ...
  if (type == TokenTypes.Base) {
    baseInAmount = amount;
    quoteInAmount = (amount * IforCalc) / baseAdjustedRatePrecision;
  } else {
